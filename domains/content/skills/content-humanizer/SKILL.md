...
**Шаг 0. Механический скан.** Если текст в файле — прогони сканер до чтения:

```bash
python3 scripts/scan_tells.py <файл>
```

Он находит вхождения регексного подмножества каталога и считает плотность на 100 слов. Это не заменяет диагностику: сканер не видит ровный ритм, отсутствие специфики и эмоциональную плоскость — половину того, что важно. Но вхождения он не пропускает и токенов не стоит, поэтому глазами ищи то, чего он не умеет, а не то, что уже нашёл.

Пороги и границы применимости — в [`references/tells-ru.md`](references/tells-ru.md). Отчёт сканера — ориентир, а не измерение: он показывает, куда смотреть, и не доказывает, что текст машинный.
...
**Шаг 4.5. Повторный скан.** Если работал с файлами — прогони сравнение:

```bash
python3 scripts/scan_tells.py <до> <после>
```

Смотри не на «убрано», а на **«добавлено при переписывании»**. Переписывающая модель заносит те же штампы обратно в парафразе: убрала «важно отметить», принесла «стоит подчеркнуть». Текст изменился, ощущение улучшилось, плотность маркеров осталась — и на глаз это почти не ловится.

Появились новые тэллы — убери их точечно, не переписывая абзац заново. Второй заход внесёт третью порцию.
